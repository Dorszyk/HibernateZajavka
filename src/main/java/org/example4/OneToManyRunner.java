...
public class OneToManyRunner {
    public static void main(String[] args) {
        ...
        Owner owner3 = ownerRepository.insertData(
                ExampleData.someOwner3(), Set.of(ExampleData.somePet1(), ExampleData.somePet2()));


        ownerRepository.listOwners().forEach(owner -> System.out.println("###Owner listing: " + owner));
        ...
        System.out.println("###Owner2:" + ownerRepository.getOwner(owner3.getId()));

        Pet newPet = Pet.builder().name("Drapek").breed(Breed.MONKEY).owner(owner1).build();
        ownerRepository.updateOwner(owner1.getId(), newPet);

        System.out.println("###Owner update:" + ownerRepository.getOwner(owner1.getId()));
        ownerRepository.listOwners().forEach(owner -> System.out.println("###Owner listing: " + owner));

        ownerRepository.deleteOwner(owner2.getId());
        ownerRepository.listOwners().forEach(owner -> System.out.println("###Owner listing: " + owner));

        HibernateUtilOneToMany.closeSessionFactory();
    }
}
